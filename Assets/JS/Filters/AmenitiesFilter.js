 ...
 *    <div class="filter-header dropdown-content">Amenities:</div>
 ...
 *
 * Behavior:
 *  - Clicking a button toggles that amenity in selectedKeywords.
 *  - Selected buttons get the "activeFilter" CSS class.
 *
 * Filtering:
 *  - matches(room) returns true if room.amenities contains ALL selectedKeywords.
 *  - If no keywords are selected, matches() returns true (filter lets everything through).
 */
class AmenitiesFilter extends FilterComponent {
   ...
   *  </div>
   *
   * Clicking a button toggles its amenity in this.selectedKeywords.
   */
  createElement() {
    // Outer wrapper
